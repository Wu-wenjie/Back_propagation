...
class Node():
    ...
    def calc_output(self):
        ...
        '''
        # 每个节点的输出算法，N元一次方程求和
        output = reduce(lambda ret,conn: ret + conn.upstream_node.output * weight, self.downstream, 0.0)
        # 结果放入激活函数
        self.output = sigmoid(output)

    def calc_hidden_layer_delata(self):
        '''
        节点属于隐藏层时，根据式4计算delta
        '''
        downstream_delta = reduce(lambda ret,conn: ret + downsteam_node.delta * conn.weight, self.downstream, 0.0)
...
class Network():
    ...
    def train(self, labels, data_set, rate, iteration):
        '''
        训练神经网络
        :param labels: 数组，训练样本标签。每个元素使一个样本的标签。
